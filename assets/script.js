...
function findItem(id) {
  return item = products.find((product) => product.productId === id)
}
...
function decreaseQuantity(id) {
  ...
  if (item.quantity === 0) {
    const productIndex = cart.indexOf(item, 0);
    cart.splice(productIndex, 1);
    //Need to locate the item in the cart and push it out of the cart without affecting the other items
    //splice() - I can't locate the corrrect item
      //Find the index of the item with the given productId, the use cart.splice(productIndex, 1) to remove the item at that index
    //
}
}
